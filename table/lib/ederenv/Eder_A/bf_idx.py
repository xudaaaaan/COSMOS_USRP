...
class Idx():
    ...
    def setup(self, fname, freq):
        ...
        Example: write_bf_awv_idx(os.path.join(dirname, 'lut/beambook/bf_rx_awv_idx'), 60.0)
        """
        freq = float(freq)
        freq = freq /1000000000
        table_tag = 'START_TABLE FREQ:' + str(freq)
        ...
        if len(bf_awv_idx) < 64 or len(bf_awv_idx) > 64:
