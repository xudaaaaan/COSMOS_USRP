...
class RxTxView(p.Page):
	...
	def poll(self, root):
		if self.pollCheck.pollVar.get():
			self.controller.updateRegisters(self)
		root.after(3000, lambda root = root: self.poll(root))
...
class ModePage(p.Page):
	def __init__(self, parent, controller, TH, RXTXMODE, terminal, *args, **kwargs):
		...
		self.beamResetButton = tk.Button(beambookPage, text="Reset beam angle", command=lambda: TH.put(lambda : self.resetBeam()))



		titleLabel.pack(side="top", fill="both", anchor="n")
		self.scale.pack(side="top", fill="both", anchor="n")
		self.beamStearingButton.pack(side="left", fill="both", anchor="n")
		self.beamResetButton.pack(side="right", fill="both", anchor="n")
	...
	def UpdateGuiTxRx(self, mode):
		...
		if mode == 'RX':
			...
			if self.mode == 'RX':
				...
				self.state_str.set("State: TX LO Leakage Calibration Running")
				MAINVIEW.nb.hide(tabs[0])
		elif mode == 'DISABLED':
