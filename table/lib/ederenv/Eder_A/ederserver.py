...
def CreateEderHandler(eder_obj, event):
	...
	class EderHandler(BaseHTTPServer.BaseHTTPRequestHandler, object):
		...
		def do_GET(self):
			...
			if request == 'txenable':
			...
			elif request == 'awvsetup':
				freq = int(split_path[2], 10)
				self.eder_obj.tx.bf.awv.setup(os.path.join(dirname, 'lut/beambook/bf_tx_awv'), freq, 0.0)
				self.eder_obj.tx.bf.idx.setup(os.path.join(dirname, 'lut/beambook/bf_tx_awv_idx'), freq)
				self.eder_obj.tx.bf.awv.dump()
			elif request == 'txawvsetupzeros':
				self.eder_obj.tx.bf.awv.setup(os.path.join(dirname, 'lut/beambook/bf_tx_awv'), 0.0, 0.0)
				self.eder_obj.tx.bf.idx.setup(os.path.join(dirname, 'lut/beambook/bf_tx_awv_idx'), 0.0)
				self.eder_obj.tx.bf.awv.wr(63, 6, 0x0)
				self.eder_obj.tx.bf.awv.dump()
			elif request == 'rxawvsetupzeros':
				self.eder_obj.rx.bf.awv.setup(os.path.join(dirname, 'lut/beambook/bf_rx_awv'), 0.0, 0.0)
				self.eder_obj.rx.bf.idx.setup(os.path.join(dirname, 'lut/beambook/bf_rx_awv_idx'), 0.0)
				...
				freq = int(split_path[2], 10)
				self.eder_obj.rx.bf.awv.setup(os.path.join(dirname, 'lut/beambook/bf_rx_awv'), freq, 0.0)
				self.eder_obj.rx.bf.idx.setup(os.path.join(dirname, 'lut/beambook/bf_rx_awv_idx'), freq)
				self.eder_obj.rx.bf.awv.dump()
			elif request == 'reset':
				self.eder_obj.reset()
				...
				n = int(split_path[3], 10);
				val = self.adc_read(mux_ch, n)
		...
		def set_channels(self, side, channels):
			...
			"""
			try:
				regVal = 2**16 + sum(map(lambda ch: 2**(int(ch, 10)-1), channels))
				print 'Setting {} channels to {}:'.format(side, channels)
				print self.eder_obj.regs.wrrd('bias_ctrl_{}'.format(side.lower()), regVal)
			except ValueError:
		...
		def adc_read(self, channel, samples):
			self.eder_obj.adc.start(channel)
			time.sleep(0.1)
			val = self.eder_obj.adc.mean(self.eder_obj.adc.dump(samples))
