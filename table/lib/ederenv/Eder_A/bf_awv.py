...
class Awv():
    ...
    def set(self, index):
        ...
        if self._txrx == self.RX:
            ...
            self.memory.awv.tgl('bf_rx_awv_ptr_set', 0x01)
        elif self._txrx == self.TX:
            self.memory.awv.wr('bf_tx_awv_ptr', index)
            self.memory.awv.tgl('bf_tx_awv_ptr_set', 0x01)
    ...
    def pack_bytes_2_bf_awv(self, i, q):
        pack = lambda i , q : ((i & 0x3f) << 8) + ((q & 0x3) << 14) + ((q & 0x3c) >> 2)
        return pack(i,q)
        
    def pack_word_2_bf_awv(self, qi):
        return self.pack_bytes_2_bf_awv(qi & 0xff,(qi >> 8) & 0xff)

    def unpack_bf_awv_2_bytes(self, data):
        return ((data & 0x3f00) >> 8, ((data & 0x0f) << 2) + ((data & 0xc000) >> 14))
    ...
    def setup(self, fname, freq, temp):
        ...
        Example: eder.tx.bf.awv.setup(os.path.join(dirname, 'lut/beambook/bf_rx_awv'), 60000000000, 25.0)
        """
        freq = float(freq)
    ...
    def wr(self, index, ant, value):
        ...
        """
        self.memory.awv.wr(self._get_element_addr(index, ant), self.pack_word_2_bf_awv(value), 2)
