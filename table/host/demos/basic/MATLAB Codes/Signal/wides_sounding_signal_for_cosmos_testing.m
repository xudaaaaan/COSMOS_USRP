...
fs = 3932.16e6/4;  % sampling frequency Hz (for DAC, used by AWG)
sc_min = 0;
sc_max = 100;
...
% IF signal
IF_f1 = round(sc_min*fs/N);  % low frequency  
IF_idx1 = floor(IF_f1/df);% index, = sc_min
IF_f2 = round(sc_max*fs/N); % high frequency, 
IF_idx2 = floor(IF_f2/df); % index, = sc_max
...
% f2 = 100e6; fix2 = floor(f2/df);% high frequency, index

if f1 == 0
    % case DC component is included, f1 = 0
    Nf = 2*(f_idx2 - f_idx1 + 1) -1; % odd number
    %Nf = 2*fix2 + 1;
    ftone = zeros(1,Nf); % frequency tones
    ftone(1:floor(Nf/2)) = (-f_idx2*df):df:-df; % negative frequency tones
    ftone(floor(Nf/2)+1) = 0; % DC 
    ftone(ceil(Nf/2)+1:end) = df:df:f_idx2*df; % positive frequency tones
    fix_eff = 1:Nf; % useful subcarrier index
else
    Nf_tot = 2*(f_idx2 - f_idx1 + 1); % even number
    ftone = zeros(1,Nf_tot);
    ftone(1:Nf_tot/2) = (-f_idx2*df):df:(-f_idx1*df);
...
f = -fs/2:df:(fs/2-df);
subplot(2,1,2);
plot(f,abs(ifftshift(fft(m_t_fs))));
xlabel('f/Hz');ylabel(' / dB');
title('x BB (time domain signal)');



figure
plot(-N/2:N/2-1,abs(ifftshift(fft(m_t_fs))));
title('x BB (time domain signal)')
...
f = 0:df:(fs-df);
figure;plot(f, 20*log10(abs(fft(x_IF))));title('x IF (time domain signal)');xlabel('Frequency [Hz]');ylabel('Magnitude [dB]')
% figure;plot(f, 20*log10(abs(fft(real(x_IF)))))
% figure;plot(f, 20*log10(abs(fft(imag(x_IF)))))
...
% signal_IF = x_IF_real';
% % save('C:\Users\zhang\OneDrive - University of Southern California\Documents\USC\WiDes\Measurement Projects\METRANS\Signal\500M_BW_single_rep_IF_no_CP.txt', 'signal_IF', '-ascii')    
% 
% figure
% pwelch(x_IF,128,64,length(x_IF),fs,'twosided');
% figure
% pwelch(x_IF_real,128,64,length(x_IF_real),fs,'twosided');
%x_IF_imag = imag(m_rep4_wcp.*exp(1j*2*pi*f_IF*t));
% figure(7)
% pwelch(x_IF_imag,128,64,length(x_IF_real),fs,'twosided');
% figure(8)
% pwelch(x_IF_real- x_IF_imag,128,64,length(x_IF_real),fs,'twosided');

%
% figure(7)
% pwelch(real(m_t),128,64,length(m_t),fs,'twosided');
% 
% figure(100)
% pwelch(hilbert(m_t),128,64,length(m_t),fs,'twosided');
% 
% 
% figure(101)
% pwelch(hilbert(m_t).*exp(1j*2*pi*f_IF*t),128,64,length(m_t),fs,'twosided');
% 
% figure(102)
% pwelch(real(hilbert(m_t).*exp(1j*2*pi*f_IF*t)),128,64,length(m_t),fs,'twosided');
% % single-sided band signal
% t=(0:length(m_t)-1).*1/fs;
% x_IF_ssb = zeros(length(m_t),2);
% x_IF_ssb(:,1) = real(m_t) .* cos(2*pi*f_IF*t);
% x_IF_ssb(:,2) = imag(hilbert(real(m_t))) .* sin(2*pi*f_IF*t);
% x_IF_ssb_assem = x_IF_ssb(:,1) - x_IF_ssb(:,2);
% figure(8)
% pwelch(x_IF_ssb(:,1),128,64,length(m_t),fs,'twosided');
% figure(9)
% pwelch(x_IF_ssb(:,2),128,64,length(m_t),fs,'twosided');