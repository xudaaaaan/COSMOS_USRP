...
class Controller():
    def __init__(self, chipRx, chipTx, view, mbtype, sn, gen, mode):
      logFile = eder_logger.EderLogger("eder.info")
      self.mode = mode
      self.chipRx = chipRx
      self.chipTx = chipTx
      self.mbtype = mbtype
      self.eder = eder.Eder(unit_name=sn, board_type=mbtype, eder_version=gen)
      self.eder.temp.init()
    ...
    def txLoLeakCal(self):
      global PAUSE_TEMP_MEAS
      PAUSE_TEMP_MEAS = True
      if self.eder.regs.rd('tx_rx_sw_ctrl') & 0b100 == 0b100: 
