    ...
    )
{
    ...
    // given), or until Ctrl-C was pressed.
        while (num_requested_samples != num_received_samps or num_requested_samples == 0) {
        // while (not stop_signal_called
            // and (num_requested_samples != num_received_samps or num_requested_samples == 0) 
            ...
            if (data_file.is_open()) {
                data_file.write((const char*)&buff.front(), num_rx_samps * sizeof(samp_type));
            }
            ...
            //         const double rate_updated = double(last_update_samps) / time_since_last_update_s;
            //         std::cout << "\t" << (rate_updated / 1e6) << " Msps" << std::endl;
            //         last_update_samps = 0;
            //         last_update       = now;
            //     }
            // }            
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    //     // this part needs to be deleted - num_samps_to_recv shouldn't be 0 so that the Rx works in finite mode
    //     if (num_samps_to_recv == 0) {
    ...
    //// ====== Start Rx ======
        ...
        //     rx_usrp, "fc64", wirefmt, write_file, spb, num_samps_to_recv, \
